          ...
          async (req, res, next) => {
            ...
            try {
              ...
              const { data: allRecords, error: fetchError } = await supabase
                .from("fuel_quantities")
                .select(
                  "id, sitename, region, refilled_date, refilled_quantity",
                );

              if (fetchError) {
                throw new Error(
                  `Failed to fetch records: ${fetchError.message}`,
                );
              }
              ...
              console.log(
                `🗑️  Total duplicate records to delete: ${duplicateIds.length}`,
              );
              ...
              for (let i = 0; i < duplicateIds.length; i += BATCH_SIZE) {
                ...
                if (deleteError) {
                  console.warn(
                    `⚠️  Batch delete error: ${deleteError.message}`,
                  );
                } else {
                  totalDeleted += batch.length;
                  console.log(
                    `✅ Deleted batch: ${batch.length} records (Total: ${totalDeleted})`,
                  );
                }
              }
              ...
              console.log(
                `   📊 Remaining: ${allRecords.length - totalDeleted} unique records`,
              );
